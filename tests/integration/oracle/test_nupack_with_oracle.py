...
from oracle import nupack

def fixed_place_round(number, DECIMAL_PLACES = 6):
    return round(number, DECIMAL_PLACES)

class TestNupackCLIWithOracle(unittest.TestCase):
    TEMPERATURE = 25.0
    ...
    def test_low_mfe_binding_affinity(self):
        ...
        self.assertEqual(fixed_place_round(4.05827286), fixed_place_round(affinity))  # value by regression

    def test_low_partition_binding_affinity(self):
        affinity = self.partition_oracle.binding_affinity("TCTACCTCTTTCCCACCTCC", "CAAAC")
        self.assertEqual(fixed_place_round(3.54004657), fixed_place_round(affinity))  # value by regression

    def test_high_mfe_binding_affinity(self):
        affinity = self.mfe_oracle.binding_affinity("CCTCTCTTACCATAAC", "GAGAGG")
        self.assertEqual(fixed_place_round(9.82990292), fixed_place_round(affinity))  # value by regression
