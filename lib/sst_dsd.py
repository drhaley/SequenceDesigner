...
def domain_orthogonal(seq,seqs,temperature,orthogonality,orthogonality_ave=-1,threaded=True):
    ...
    if threaded:
        ...
        ss = binding(seq, seq, temperature, negate) #not sticky to itself
        log_energy(ss)
        if ss > orthogonality: 
            return False
        wsws = binding(wc(seq), wc(seq), temperature, negate)   #complement is not sticky to itself
        log_energy(wsws)
        if wsws > orthogonality: 
            return False
    energy_sum = 0.0
    for altseq in seqs:          #new candidate sequence = seq.  an already existing sequence = altseq.
        if threaded:
